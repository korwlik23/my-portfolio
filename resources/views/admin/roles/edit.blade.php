...
<div class="max-w-3xl">
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6">
        <form method="POST" action="/admin/roles/{{ $role->id }}" class="space-y-6">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-medium mb-1.5">{{ __('messages.role_name_label') }}</label>
                <input type="text" name="name" value="{{ $role->name }}" required class="w-full px-4 py-2.5 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm">
            </div>
                        ...
                        <div class="border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                            ...
                            <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <label class="flex items-center gap-3 p-3 border border-zinc-200 dark:border-zinc-800 rounded-xl cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition">
                                    ...
                                    <span class="text-sm font-medium">{{ __('messages.perm_' . str_replace(['_', '.'], '_', $perm)) }}</span>
                                </label>
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="flex gap-3 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                <button type="submit" class="px-6 py-2.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-sm font-semibold hover:bg-zinc-800 transition">{{ __('messages.save') }}</button>
