...
<div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm">
            <h2 class="text-lg font-semibold mb-6">{{ __('messages.login') }}</h2>
            @if(session('error'))
            ...
            <form method="POST" action="/login" class="space-y-4">
                ...
                <div class="flex items-center justify-between mt-2">
                    <a href="{{ route('password.request') }}" class="text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition">
                        {{ __('messages.forgot_password') ?? 'Forgot password?' }}
                    </a>
                </div>
                <button type="submit" class="w-full mt-4 py-2.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-sm font-semibold hover:bg-zinc-800 dark:hover:bg-zinc-100 transition active:scale-[0.98]">{{ __('messages.login') }}</button>
            </form>
            <div class="text-center mt-4">
                <p class="text-sm text-zinc-500">
                    {{ __('messages.no_account') }} 
                    <a href="{{ route('register') }}" class="text-zinc-900 dark:text-white font-semibold hover:underline">
